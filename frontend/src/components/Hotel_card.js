...
function HotelCard({ imageUrl, name, ratings, location, priceRange }) {
  return (
    <div className="flex items-center border border-gray-200 rounded-lg p-4 shadow-md">
      <div className="flex-shrink-0 mr-4">
        <img src={imageUrl} alt="Hotel" className="h-24 w-24 rounded-lg" />
      </div>
      <div>
        ...
        <p className="text-sm font-semibold mt-1">{priceRange}</p>
      </div>
    </div>
  );
}
